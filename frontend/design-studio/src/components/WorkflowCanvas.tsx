...
interface WorkflowCanvasProps {
  ...
  // onWorkflowChange: (workflow: WorkflowDefinition) => void // TODO: Implement canvas-to-workflow updates
}
...
export default function WorkflowCanvas({ workflow }: WorkflowCanvasProps): JSX.Element {
  ...
  // Convert workflow steps to edges
  ...
  const handleNodesChange = useCallback(
    (changes: NodeChange[]) => {
      ...
      // For now, we don't update workflow on position changes
      // Could be extended to save positions in step config
    },
    [onNodesChange]
  )
  ...
  const handleEdgesChange = useCallback(
    (changes: EdgeChange[]) => {
      ...
      // For now, we don't update workflow on edge changes
      // Could be extended to update step dependencies
    },
    [onEdgesChange]
  )
      ...
      <Card variant="outlined" sx={{ p: 2, mb: 1 }}>
        <CardContent sx={{ '& p': { mb: 0 } }}>
          <Typography variant="subtitle2" color="textSecondary">
            Drag nodes, connect edges, and inspect the workflow path. Use the canvas to prototype workflow execution flows with connectors, agents, and branches.
          </Typography>
        </CardContent>
      </Card>
        ...
        <Box sx={{ width: 280, display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Card variant="outlined" sx={{ p: 2, flex: 1 }}>
            ...
            <Typography variant="body2" color="textSecondary">
              Connect new nodes by dragging from the output handle of any node. Use standard modifiers to pan and zoom in the canvas.
            </Typography>
          </Card>
        </Box>
      </Box>
